...
| # | 项 | 现状问题 | 改造动作 | 验收 |
|---|---|---|---|---|
| 1 | **估值分位真正算出来** ✅ | ~~没算分位就标估值分位~~ → `fetch_valuation_history.js` + `score_and_report.js` 已落地 | `RPT_VALUEANALYSIS_DET`×`DIVIDEND_MAIN` 算 TTM 股息率分位 + PB 分位；≥80/无息时PB≤20 才「分批≤5%」，否则观望 | 报告出现真实分位数且与动作一致（如 600900：股息分位~64→观望） |
