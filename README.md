...
| # | 项 | 现状问题 | 改造动作 | 验收 |
|---|---|---|---|---|
| 1 | **估值分位次路径已删除** | 曾用股息分位≥80 驱动「分批≤5%」，易选出相对历史高息但非理想买点的票 | 买卖只认笑傲牛熊四阶段（阶段2 入场口径）；估值分位脚本已删 | §1 无理想买点时 K=0；信号来源不得写「估值分位」 |
| 2 | **股息隐含目标价用中枢，不用行业基准凑** | `divTargetPrice` 只用「现价×股息/行业基准」 | 优先：近 12 月 DPS ÷ 近 5 年 TTM 股息中位数；不足年限再降级到基准，并标注 | 附录每票写明分母来源 |
| 3 | **阶段分析目标价复权一致** | 均线/平台若用后复权，现价多半是未复权/前复权，不可比 | 阶段判定与目标价统一走前复权日线（`calc_stage.js`），后复权只作收益观察 | 不再出现「现价 30、MA30W 180」类错位 |
